...
module.exports.run = async (bot, message, args) => {
    let botprefix = "m!"
            ...
            const bassboostEnabled = bot.player.getQueue(message.guild.id).filters.subboost;
            if(!bassboostEnabled){
                try {
                    ...
                    .setAuthor(`${message.author.tag} has SubBoost the current song`, `${message.author.displayAvatarURL()}`)
                    .setTitle('Successfully SubBoost')
                    .setDescription(`Music has been successfully SubBoost\n Song SubBoost: [${song.name}](${song.url})\n To simply Disable SubBoost: \`\`${botprefix}SubBoost\`\``)
                    .setColor('BLUE')
            ...
            try {
                ...
                .setAuthor(`${message.author.tag} has Disabled SubBoost the current song`, `${message.author.displayAvatarURL()}`)
                .setTitle('Successfully Disabled SubBoost')
                .setDescription(`Music has been successfully Disabled SubBoost\n Song SubBoost Disabled: [${song.name}](${song.url})\n To simply Enable SubBoost: \`\`${botprefix}SubBoost\`\``)
                .setColor('BLUE')
                .setFooter('Onyx Music System');
...
module.exports.config = {
    name: "subboost",
    category: 'music',
    description: "SubBoost's Your music",
    usage: "<prefix>subboost",
